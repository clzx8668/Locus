...
class _TemplateManagementPageState extends State<TemplateManagementPage> {
  ...
  Widget _buildTemplateItem(
      AiTemplate t, int index, AppColorsExtension colors, bool isDark) {
    return Padding(
      key: ValueKey(t.id),
      ...
      builder: (ctx) {
        return StatefulBuilder(builder: (ctx, setDialogState) {
          ...
          return AlertDialog(
            ...
            title: Text(
              isEdit ? '编辑模板' : '新建模板',
              style: AppTypography.h3.copyWith(color: dialogColors.textPrimary),
            ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextField(
                    controller: nameCtrl,
                    style:
                        TextStyle(fontSize: 14, color: dialogColors.textPrimary),
                    decoration: InputDecoration(
                      hintText: '例如：总结全文要点',
                      hintStyle: TextStyle(
                          fontSize: 14, color: dialogColors.textTertiary),
                      filled: true,
                  ...
                  TextField(
                    ...
                    minLines: 3,
                    style:
                        TextStyle(fontSize: 13, color: dialogColors.textPrimary),
                    decoration: InputDecoration(
                      hintText: '输入 AI 提示词模板...',
                      hintStyle: TextStyle(
                          fontSize: 13, color: dialogColors.textTertiary),
                      filled: true,
                      fillColor: dialogColors.surface2,
                      contentPadding: const EdgeInsets.symmetric(
                          horizontal: 12, vertical: 10),
            ...
            actions: [
              ...
              FilledButton(
                ...
                onPressed: () async {
                  ...
                  if (isEdit) {
                    await _repo.update(template.id,
                        icon: selectedEmoji, name: name, prompt: prompt);
                  } else {
                    await _repo.insert(selectedEmoji, name, prompt);
                  }
                  nav.pop();
                },
    ...
    showDialog(
      ...
      builder: (ctx) {
        ...
        return AlertDialog(
          ...
          content: Text('确定要删除「${t.name}」吗？此操作不可恢复。',
              style:
                  TextStyle(fontSize: 14, color: dColors.textSecondary)),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(ctx),
              child: Text('取消',
  ...
  void _exportTemplates() async {
    final templates = await _db().watchAllTemplates().first;
        ...
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text('粘贴 JSON 内容开始导入',
                  style: TextStyle(
                      fontSize: 13, color: dColors.textSecondary)),
              const SizedBox(height: 12),
              TextField(
                controller: ctrl,
                maxLines: 5,
                style:
                    TextStyle(fontSize: 13, color: dColors.textPrimary),
                decoration: InputDecoration(
                  hintText: '在此粘贴 JSON...',
                  hintStyle: TextStyle(
                      fontSize: 13, color: dColors.textTertiary),
                  filled: true,
  ...
  AppDatabase _db() => getIt<AppDatabase>();
}
